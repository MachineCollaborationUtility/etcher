...
electron.app.on('ready', async() => {
  await server(electron.app)

  // No menu bar
  electron.Menu.setApplicationMenu(null)

  mcuWindow = new electron.BrowserWindow({
    width: 992,
    height: 800,
    useContentSize: true,
    show: false,
  ...
  etcherWindow = new electron.BrowserWindow({
    width: 800,
    height: 380,
    useContentSize: true,
    show: false,
  ...
  mcuWindow.on('ready-to-show', etcherWindow.show)

  // Allow the etcher window to be opened and closed multiple times
  etcherWindow.on('close', (event) => {
  ...
  etcherWindow.webContents.openDevTools()
  etcherWindow.on('focus', () => {
  ...
  etcherWindow.webContents.executeJavaScript('require(\'electron\').webFrame.setZoomLevelLimits(1, 1);')

  // Prevent external resources from being loaded (like images)
  // when dropping them on the WebView.
  // See https://github.com/electron/electron/issues/5919
  etcherWindow.webContents.on('will-navigate', (event) => {
    event.preventDefault()
  })

  etcherWindow.loadURL(`file://${path.join(__dirname, 'index.html')}`)
  mcuWindow.loadURL('http://localhost:9000')

  electron.ipcMain.on('open-etcher', (event, arg) => {
    etcherWindow.show()
  })

  electron.ipcMain.on('download-mcu', (event, arg) => {
    download(electron.BrowserWindow.getFocusedWindow(), arg)
    .then((dl) => {
      const imagePath = dl.getSavePath()
      event.sender.send('download-mcu', { imagePath })
    })
    .catch((error) => {
      event.sender.send('download-mcu', { error })
    })
  })
})
