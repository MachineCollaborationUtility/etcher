...
module.exports = function (
  ...
  this.checkForNewImage = (allowDownload = false) => {
    ...
    const request = new Request('http://localhost:8080/example-github.json')
    ...
    .then(json => {
      ...
      try {
        ...
        if (mcuImages.length > 0) {
          ...
          if (semver.compare(newest, latestDownload) <= 0) {
        ...
        //   console.log('stick with ol faithful');
        // }
        // const fileName = json.assets[0].name;
        // const ichemagePath = path.join(remote.app.getPath('downloads'), fileName)

        // // If that image is available in the 'Downloads' folder, then select it
        // if (fs.existsSync(imagePath)) {
        //   this.selectImageByPath(imagePath)
        // } else {
        //   // If the image isn't available then download it, then select it
        //   console.log('download it', json.assets[0].browser_download_url);
        // }
      } catch (ex) {
  ...
  this.openImageSelector = () => {
    try {
      var basepath = remote.app.getPath('downloads');
